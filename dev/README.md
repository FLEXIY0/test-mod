...
## Внимание: удаление классов

`dev/build.sh` только **добавляет/заменяет** классы в jar, но не удаляет. Если
удаляешь файл из `dev/src/`, его старый `.class` (и внутренние классы) остаётся
в jar «сиротой». Вычисти вручную: `zip -d InDevPlus/libraries/customjar-1.jar
'net/minecraft/.../Имя*'`, затем пересобери.
