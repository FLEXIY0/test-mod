...
import net.minecraft.client.c.a.C_u;
...
extends C_c {
    ...
    public C_e(C_b c_b, C_b c_b2) {
        ...
        for (n2 = 0; n2 < this.size; ++n2) {
            for (n = 0; n < 9; ++n) {
                this.addSlot(new C_u(c_b2, n + n2 * 9, 8 + n * 18, 18 + n2 * 18));
            }
        }
        for (n2 = 0; n2 < 3; ++n2) {
            for (n = 0; n < 9; ++n) {
    ...
    public ItemStack getStackInSlot(int n) {
        ...
        if (c_g != null && c_g.getHasStack()) {
            ...
            if (n < this.size * 9) {
                if (!this.isSlotFree(itemStack2, this.size * 9, this.slots.size(), false)) {
                    return null;
                }
            } else if (itemStack.a() == Item.book || itemStack.a() == Item.bookAndQuill || itemStack.a() == Item.writtenBook) {
                if (!this.isSlotFree(itemStack2, 0, this.size * 9, false)) {
                    return null;
                }
            } else {
                return null;
