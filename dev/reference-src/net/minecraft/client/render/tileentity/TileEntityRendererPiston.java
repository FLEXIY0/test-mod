...
extends TileEntitySpecialRenderer {
    ...
    public void renderPiston(C_j c_j, float f, float f2, float f3, float f4) {
        ...
        if (c_x != null && c_j.getProgress(f4) < 1.0f) {
            ...
            if (c_x == Block.pulleyExtension && c_j.getProgress(f4) < 0.5f) {
                this.blockRenderer.renderPistonExtensionAllFaces(c_x, c_j.b, c_j.c, c_j.d, false);
            } else if (c_j.shouldRenderHead() && !c_j.isExtending()) {
                Block.pulleyExtension.setHeadTexture(((BlockPistonBase)c_x).getPistonExtensionTexture());
                this.blockRenderer.renderPistonExtensionAllFaces(Block.pulleyExtension, c_j.b, c_j.c, c_j.d, c_j.getProgress(f4) < 0.5f);
                Block.pulleyExtension.clearHeadTexture();
