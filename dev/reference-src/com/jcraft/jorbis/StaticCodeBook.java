...
class StaticCodeBook {
    ...
    int pack(Buffer buffer) {
        ...
        if (n2 != 0) {
            ...
            for (n = 1; n < this.entries; ++n) {
                int n3 = this.lengthlist[n];
                int n4 = this.lengthlist[n - 1];
                if (n3 <= n4) continue;
                while (n4 < n3) {
                    buffer.write(n - n2, Util.ilog(this.entries - n2));
                    n2 = n;
                    ++n4;
                }
            }
            ...
            buffer.write(0, 1);
            for (n = 0; n < this.entries && this.lengthlist[n] != 0; ++n) {
            }
            if (n == this.entries) {
                buffer.write(0, 1);
                for (n = 0; n < this.entries; ++n) {
                    buffer.write(this.lengthlist[n] - 1, 5);
                }
            } else {
                buffer.write(1, 1);
                for (n = 0; n < this.entries; ++n) {
                    if (this.lengthlist[n] == 0) {
                        buffer.write(0, 1);
                        continue;
                    }
                    buffer.write(1, 1);
        ...
        switch (this.maptype) {
            ...
            case 2: {
                ...
                for (n = 0; n < n2; ++n) {
                    buffer.write(Math.abs(this.quantlist[n]), this.q_quant);
                }
                break;
            }
            default: {
    ...
    private int maptype1_quantvals() {
        int n = MathHelper.a(Math.pow(this.entries, 1.0 / (double)this.dim));
        while (true) {
            int n2 = 1;
            int n3 = 1;
    ...
    static long float32_pack(float f) {
        ...
        int n2 = MathHelper.a(Math.log(f) / Math.log(2.0));
        int n3 = (int)Math.rint(Math.pow(f, 20 - n2));
        n2 = n2 + 768 << 21;
        return n | n2 | n3;
    }

    static float float32_unpack(int n) {
        float f = n & 0x1FFFFF;
    ...
    static float ldexp(float f, int n) {
