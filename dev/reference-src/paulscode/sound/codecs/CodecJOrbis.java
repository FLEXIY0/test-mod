...
implements ICodec {
    ...
    private boolean readHeader() {
        ...
        int n2 = 0;
        while (n2 < 2) {
            while (n2 < 2 && (n = this.joggSyncState.pageout(this.joggPage)) != 0) {
                if (n != 1) continue;
                this.joggStreamState.pagein(this.joggPage);
                while (n2 < 2 && (n = this.joggStreamState.packetout(this.joggPacket)) != 0) {
                    if (n == -1) {
    ...
    private byte[] decodeCurrentPacket() {
        ...
        int n3 = this.convertedBufferSize / (this.jorbisInfo.channels << 1);
        for (n = 0; n < this.convertedBufferSize && (n2 = this.jorbisDspState.synthesis_pcmout(this.pcmInfo, this.pcmIndex)) > 0; n += n2 * this.jorbisInfo.channels << 1) {
            if (n2 >= n3) {
                n2 = n3;
            }
            for (int i = 0; i < this.jorbisInfo.channels; ++i) {
                ...
                for (int j = 0; j < n2; ++j) {
                    ...
                    byArray[n + n4 + 1] = (byte)(n5 >>> 8);
                    n4 += 2 * this.jorbisInfo.channels;
                }
            }
            this.jorbisDspState.synthesis_read(n2);
        }
        byArray = CodecJOrbis.trimArray(byArray, n);
        return byArray;
    }
