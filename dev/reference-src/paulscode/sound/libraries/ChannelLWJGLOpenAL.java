...
public class ChannelLWJGLOpenAL extends Channel {
   ...
   public boolean preLoadBuffers(LinkedList var1) {
      if (this.errorCheck(this.channelType != 1, "Buffers may only be queued for streaming sources.")) {
         ...
         boolean var3;
         if (var3 = this.playing()) {
            AL10.alSourceStop(this.ALSource.get(0));
            this.checkALError();
         }

         int var2;
         if ((var2 = AL10.alGetSourcei(this.ALSource.get(0), 4118)) > 0) {
            IntBuffer var6;
            AL10.alGenBuffers(var6 = BufferUtils.createIntBuffer(var2));
            if (this.errorCheck(this.checkALError(), "Error clearing stream buffers in method 'preLoadBuffers'")) {
               return false;
            }

            AL10.alSourceUnqueueBuffers(this.ALSource.get(0), var6);
         ...
         IntBuffer var7;
         AL10.alGenBuffers(var7 = BufferUtils.createIntBuffer(var1.size()));
         if (this.errorCheck(this.checkALError(), "Error generating stream buffers in method 'preLoadBuffers'")) {
            return false;
         } else {
            for (int var8 = 0; var8 < var1.size(); var8++) {
               ((Buffer)this.bufferBuffer).clear();
               this.bufferBuffer.put((byte[])var1.get(var8), 0, ((byte[])var1.get(var8)).length);
               ((Buffer)this.bufferBuffer).flip();

               try {
                  AL10.alBufferData(var7.get(var8), this.ALformat, this.bufferBuffer, this.sampleRate);
            ...
            try {
               AL10.alSourceQueueBuffers(this.ALSource.get(0), var7);
   ...
   public int feedRawAudioData(byte[] var1) {
      if (this.errorCheck(this.channelType != 1, "Raw audio data can only be fed to streaming sources.")) {
         ...
         ByteBuffer var4 = ByteBuffer.wrap(var1, 0, var1.length);
         IntBuffer var2;
         int var3;
         if ((var3 = AL10.alGetSourcei(this.ALSource.get(0), 4118)) > 0) {
            AL10.alGenBuffers(var2 = BufferUtils.createIntBuffer(var3));
            ...
            AL10.alGenBuffers(var2 = BufferUtils.createIntBuffer(1));
            if (this.errorCheck(this.checkALError(), "Error generating stream buffers in method 'preLoadBuffers'")) {
               return -1;
            }
         }

         AL10.alBufferData(var2.get(0), this.ALformat, var4, this.sampleRate);
         if (this.checkALError()) {
            return -1;
         } else {
            AL10.alSourceQueueBuffers(this.ALSource.get(0), var2);
